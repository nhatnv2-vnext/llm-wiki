...
- **Parser cho Node/Angular** → dùng **ts-morph**, KHÔNG dùng GitNexus (GitNexus không resolve được path alias `@/` và monorepo).
- **Cài GitNexus** → `sudo npm install -g gitnexus` (global), KHÔNG clone source vào vault. Output `.md` đặt trong `02_Wiki/05_Code_Graph/<project>/`.
